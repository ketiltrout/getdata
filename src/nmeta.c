...
unsigned int gd_get_nmfields(DIRFILE* D, const char* parent)
{
  dtrace("%p", D);
...
unsigned int gd_get_nmvectors(DIRFILE* D, const char* parent)
{
  dtrace("%p", D);
...
unsigned int gd_get_nmfields_by_type(DIRFILE* D, const char* parent,
    gd_entype_t type)
{
  unsigned int r = 0;
  int i;
