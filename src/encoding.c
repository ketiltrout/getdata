    ...
    const char* name)
{
  ...
  sprintf(symbol, "_GD_%s%s", enc->affix, name);
  func = dlsym(lib, symbol);
...
int _GD_MissingFramework(int encoding, unsigned int funcs)
{
  ...
  if (encode[encoding].provides) {
    char library[FILENAME_MAX];
    void* lib;

    /* make the library name */
    sprintf(library, "libgetdata%s-%s.so", encode[encoding].affix,
        PACKAGE_VERSION);
    library[10] -= 'A' - 'a';

    /* open */
    if ((lib = dlopen(library, RTLD_LAZY | RTLD_LOCAL)) == NULL) {
      dreturn("%i", 1);
      return 1;
    }

    /* Try to resolve the symbols */
    if (encode[encoding].provides & GD_EF_OPEN)
      encode[encoding].open = _GD_ResolveSymbol(lib, encode + encoding, "Open");
    if (encode[encoding].provides & GD_EF_CLOSE)
      encode[encoding].close = _GD_ResolveSymbol(lib, encode + encoding,
          "Close");
    if (encode[encoding].provides & GD_EF_TOUCH)
      encode[encoding].touch = _GD_ResolveSymbol(lib, encode + encoding,
          "Touch");
    if (encode[encoding].provides & GD_EF_SEEK)
      encode[encoding].seek = _GD_ResolveSymbol(lib, encode + encoding, "Seek");
    if (encode[encoding].provides & GD_EF_READ)
      encode[encoding].read = _GD_ResolveSymbol(lib, encode + encoding, "Read");
    if (encode[encoding].provides & GD_EF_SIZE)
      encode[encoding].size = _GD_ResolveSymbol(lib, encode + encoding, "Size");
    if (encode[encoding].provides & GD_EF_WRITE)
      encode[encoding].write = _GD_ResolveSymbol(lib, encode + encoding,
          "Write");
    if (encode[encoding].provides & GD_EF_SYNC)
      encode[encoding].sync = _GD_ResolveSymbol(lib, encode + encoding, "Sync");
    if (encode[encoding].provides & GD_EF_UNLINK)
      encode[encoding].unlink = _GD_ResolveSymbol(lib, encode + encoding,
          "Unlink");
    if (encode[encoding].provides & GD_EF_TEMP)
      encode[encoding].temp = _GD_ResolveSymbol(lib, encode + encoding, "Temp");

    /* we tried our best, don't bother trying again */
    encode[encoding].provides = 0;
  }
