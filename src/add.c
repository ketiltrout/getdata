...
static void _GD_AddSpec(DIRFILE* D, const char* line, const char* parent,
    ...
    int fragment_index) gd_nothrow
{
  ...
  if (D->flags & GD_INVALID)
    _GD_SetError(D, GD_E_BAD_DIRFILE, 0, NULL, 0, NULL);
  else if (n_fields < 1 || n_fields > GD_MAX_LINCOM)
    ...
    const char* in_field, const double* a, int fragment_index) gd_nothrow
{
  ...
  if (D->flags & GD_INVALID)
    _GD_SetError(D, GD_E_BAD_DIRFILE, 0, NULL, 0, NULL);
  else if (poly_ord < 1 || poly_ord > GD_MAX_LINCOM)
    ...
    int poly_ord, const char* in_field, const double* a) gd_nothrow
{
  ...
  if (D->flags & GD_INVALID)
    _GD_SetError(D, GD_E_BAD_DIRFILE, 0, NULL, 0, NULL);
  else if (poly_ord < 1 || poly_ord > GD_MAX_POLYORD)
    _GD_SetError(D, GD_E_BAD_ENTRY, GD_E_ENTRY_POLYORD, NULL, poly_ord, NULL);
  else
    _GD_ClearError(D);
...
/* add an alias */
static void _GD_AddAlias(DIRFILE *restrict D, const char *restrict parent,
