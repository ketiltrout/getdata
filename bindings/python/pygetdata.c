...
PyMODINIT_FUNC initpygetdata(void)
{
  ...
  mod = Py_InitModule3("pygetdata", GetDataMethods,
      ...
      "gd_putdata(3),\nwhich is required in the C API, are typically optional, "
      "as pygetdata can\ndetermine the input data type by itself, and convert "
      ...
      "it can.  For gd_getdata(3) and\nsimilar, the C API types are converted "
      "to Python types as follows:\n\n"
